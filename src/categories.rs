...
use crate::{Args, keys::CATEGORY_KEY};
...
pub fn set_categories_toml(package: &mut Item, args: &Args) {
    let have_categories = (package.get(CATEGORY_KEY).is_some() && package[CATEGORY_KEY].is_array() && !package[CATEGORY_KEY].as_array().unwrap().is_empty());
