...
use crate::{Args, keys::{LICENSE_KEY, LICENSE_FILE_KEY}};
    ...
    if !args.non_interactive {
        ...
        if t == 1 {
          ...
          package[LICENSE_KEY] = toml_edit::Item::Value(Value::String(Formatted::new(t.to_string())));
            ...
            package[LICENSE_FILE_KEY] = toml_edit::Item::Value(Value::String(Formatted::new("LICENSE.txt".to_string())));
            println!("The License File has been set to 'LICENSE.txt'. Update as necessary and/or make sure the file exists before publishing. Press enter to continue");
            input::<String>().get();
        }
    } else {
        package[LICENSE_KEY] = toml_edit::Item::Value(Value::String(Formatted::new(License::MITApache.to_string())));
    }
  }
}
